...
- short_term: what happened recently (last N items)
...
class Memory:
    ...
    def remember(self, content: str, kind: str = "note", important: bool = False, **meta):
        item = MemoryItem(ts=self._now(), kind=kind, content=content, meta=dict(meta))

        # Always keep a rolling short-term log
    ...
    def recall_recent(self, kind: str | None = None, limit: int = 10) -> List[MemoryItem]:
        items = self.short_term
        if kind:
            items = [i for i in items if i.kind == kind]
        return items[-limit:]

    def recall_important(self, kind: str | None = None, limit: int = 20) -> List[MemoryItem]:
